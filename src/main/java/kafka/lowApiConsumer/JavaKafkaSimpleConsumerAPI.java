 ...
 *
 * Lower Level Consumer API：通过直接操作底层API获取数据的方式获取Kafka中的数据，需要自行给定分区、偏移量等属性。
 * 优点：可操作性强；缺点：代码相对而言比较复杂。(入口类：SimpleConsumer)
 *
        ...
        try {
            ...
            while (ever || maxReads > 0) {
                ...
                for (MessageAndOffset messageAndOffset : response.messageSet(topic, partitionID)) {
                    ...
                    payload.get(bytes);
                    System.out.println("offset："+ currentOffset + ",  message:" + new String(bytes, "UTF-8")+
